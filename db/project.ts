...
export async function listProjects(supabase: SupabaseClient) {
  const { data, error } = await supabase
    .from('projects')
    .select(
      'title, id, creator, slug, blurb, stage, funding_goal, profiles(*), bids(id), txns(*), comments(id), rounds(title, slug)'
    )
...
export async function getFullProjectsByRound(
  ...
  const { data, error } = await supabase
    .from('projects')
    .select(
      'title, id, creator, slug, blurb, stage, funding_goal, profiles(*), bids(*), txns(*), comments(*), rounds(title, slug)'
    )
    .eq('round', roundTitle)
  if (error) {
    throw error
