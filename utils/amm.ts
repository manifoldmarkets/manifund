...
export function calculateAMMPorfolio(txns: Txn[], ammId: string) {
  const ammTxns = txns.filter((txn) => txn.from_id === ammId || txn.to_id === ammId)
...
export function calculateBuyShares(dollarAmount: number, ammShares: number, ammUSD: number) {
  const uniswapProduct = ammUSD * ammShares
  return ammShares - uniswapProduct / (ammUSD + dollarAmount)
}

export function calculateSellPayout(shares: number, ammShares: number, ammUSD: number) {
  const uniswapProduct = ammUSD * ammShares
  return ammUSD - uniswapProduct / (ammShares + shares)
}

export function ammSharesAtValuation(uniswapProduct: number, valuation: number) {
...
export function calculateValuationAfterTrade(
  ...
  const ammSharesAtTrade = valuationAtTrade
    ...
    : ammShares
  const ammUSDAtTrade = valuationAtTrade ? uniswapProduct / ammSharesAtTrade : ammUSD
  ...
  const ammUSDAfterTrade = isBuying ? ammUSDAtTrade + usdInTrade : ammUSDAtTrade - usdInTrade
...
export const getTradeErrorMessage = (
  ...
  if (isLimitOrder) {
    if (!limitValuation) {
      ...
      return `Valuation must be greater than ${formatMoneyPrecise(minValuation)}`
    } else if (usingAmm && modeId === 'sell' && limitValuation < currentValuation) {
      return 'For sell limit orders, valuation must be greater than current valuation'
    } else if (usingAmm && modeId === 'buy' && limitValuation > currentValuation) {
  ...
  } else if (modeId === 'sell' && percentEquity * TOTAL_SHARES > userSellableShares) {
...
export function calculateTradePoints(txns: TxnAndProfiles[], ammId: string) {
  const ammTxns = txns.filter(
    (txn) => txn.bundle !== null && (txn.to_id === ammId || txn.from_id === ammId)
  )
  const usdTxns = ammTxns.filter((txn) => txn.token === 'USD')
  const sortedUsdTxns = sortBy(usdTxns, ['created_at'])
  const tradePoints = Object.fromEntries(sortedUsdTxns.map((txn) => [txn.bundle, {} as TradePoint]))
  sortedUsdTxns.forEach((txn) => {
    const point = tradePoints[txn.bundle as string]
    const sharesTxn = ammTxns.find((t) => t.bundle === txn.bundle && t.token !== 'USD')
    if (!sharesTxn) return
    const ammTxnsSoFar = ammTxns.filter((t) =>
      isBefore(sub(new Date(t.created_at), { seconds: 1 }), new Date(txn.created_at))
    )
