...
const COUNTED = ['mercury_grants', 'mercury_mox', 'stripe_manifund', 'stripe_mox', 'coinbase', 'other']
const BANK_CATEGORIES = ['internal_transfer', 'stripe_payout', 'payroll', 'rent', 'cleaning', 'card_autopay', 'wire_fee', 'direct_grant', 'ops', 'other_expenses', 'revenue', 'other']
const NC_CATEGORIES = ['pass_through_grant', 'direct_grant', 'investor_pass_through', 'ops', 'other']
...
function renderImport() {
  ...
  main.innerHTML = `
    ...
    <div class="section"><h2>Imported bank txns</h2>
      ${Object.entries(counts).map(([s, n]) => `<span class="badge">${SOURCES[s] || s}: ${n}</span> `).join('') || '<span class="muted">none yet</span>'}
    </div>
    <div class="section"><h2>Add manual bank txn</h2>
      ...
      <select id="mt-source">${Object.entries(SOURCES).map(([k, v]) => `<option value="${k}">${v}</option>`).join('')}</select>
    ...
    for (const file of files) {
      ...
      try {
        ...
        if (res.needsMapping) {
          ...
          const sel = (id) => `<select id="${id}">${res.headers.map((h) => `<option>${esc(h)}</option>`).join('')}</select>`
          $('#import-log').innerHTML += `<div id="map-ui">date ${sel('m-date')} amount ${sel('m-amount')} description ${sel('m-desc')} id (optional) <select id="m-id"><option value="">—</option>${res.headers.map((h) => `<option>${esc(h)}</option>`).join('')}</select>
            <button class="action" id="m-go">Import with mapping</button></div>`
          $('#m-go').onclick = async () => {
            const mapping = { dateCol: $('#m-date').value, amountCol: $('#m-amount').value, descriptionCol: $('#m-desc').value, idCol: $('#m-id').value || undefined }
  ...
  const importSummary = (name, r) =>
    `<b>${esc(name)}</b> (${r.format}): +${r.inserted} new, ${r.skipped} duplicate, ${r.updated || 0} updated, ${r.dropped} dropped (pending/failed) · ${Object.entries(r.sourceCounts).map(([s, n]) => `${SOURCES[s] || s} ${n}`).join(', ')}${r.dateRange ? ` · ${r.dateRange[0]} → ${r.dateRange[1]}` : ''}`
  ...
  $('#mt-add').onclick = async () => {
    ...
    if (!body.date || !body.amount || !body.description) return status('date, amount and description required', true)
  ...
  $('#automatch').onclick = async () => {
    ...
    status(`auto-match: ${r.newMatches} new matches (${Object.entries(r.phaseCounts).map(([k, v]) => `${k} ${v}`).join(', ')})`)
    await loadState()
  }
...
function renderMatching() {
  ...
  if (filters.bankText) bank = bank.filter((t) => (t.description + ' ' + (t.counterparty || '')).toLowerCase().includes(filters.bankText.toLowerCase()))
  ...
  if (filters.platText) plat = plat.filter((t) => (t.username + ' ' + t.fullName + ' ' + t.email).toLowerCase().includes(filters.platText.toLowerCase()))
  if (filters.hideSmall) plat = plat.filter((t) => t.amount >= 100)
  if (suggested.length) {
    const rank = new Map(suggested.map((id, i) => [id, i]))
    plat.sort((a, b) => (rank.has(a.id) ? rank.get(a.id) : 1e9) - (rank.has(b.id) ? rank.get(b.id) : 1e9) || b.amount - a.amount)
  } else plat.sort((a, b) => b.amount - a.amount)

  const CAP = 400
  const matchGroups = S.matches
    .filter((m) => m.bankTxnIds.some((id) => { const t = S.bankTxns.find((x) => x.id === id); return t && inWin(t.date) }) ||
                   m.platformTxnIds.some((id) => { const t = S.platformTxns.find((x) => x.id === id); return t && inWin(t.created_at.slice(0, 10)) }))
    .slice()
    .reverse()
  ...
  main.innerHTML = `
    <div class="panes">
      <div class="pane">
        ...
        <div class="filters">
          <select id="f-source"><option value="">all accounts</option>${Object.entries(SOURCES).map(([k, v]) => `<option value="${k}" ${filters.bankSource === k ? 'selected' : ''}>${v}</option>`).join('')}</select>
        ...
        <div class="tbl-wrap"><table>
          ...
          ${bank.slice(0, CAP).map((t) => `
            <tr><td><input type="checkbox" data-bank="${t.id}" ${selBank.has(t.id) ? 'checked' : ''}/></td>
              ...
              <td>${t.category ? `<span class="badge cat">${t.category}${t.categorySource === 'auto' ? '·a' : ''}</span>` : ''}</td></tr>`).join('')}
        </table></div>
      </div>
      <div class="pane">
        ...
        <div class="tbl-wrap"><table>
          ...
          ${plat.slice(0, CAP).map((t) => `
            <tr class="${suggested.includes(t.id) ? 'suggested' : ''}"><td><input type="checkbox" data-plat="${t.id}" ${selPlat.has(t.id) ? 'checked' : ''}/></td>
              ...
              <td class="desc">${esc(t.username)} <span class="muted">${esc(t.fullName)}</span></td></tr>`).join('')}
        </table></div>
      </div>
    </div>
    <div class="section" style="margin-top:12px"><h2>Match groups in window (${matchGroups.length})</h2>
      <div class="tbl-wrap"><table>
        ${matchGroups.slice(0, 200).map((m) => {
          const bts = m.bankTxnIds.map((id) => S.bankTxns.find((t) => t.id === id)).filter(Boolean)
          const pts = m.platformTxnIds.map((id) => S.platformTxns.find((t) => t.id === id)).filter(Boolean)
          return `<tr>
  ...
  const updateSums = () => {
    const bNet = [...selBank].reduce((s, id) => s + (S.bankTxns.find((t) => t.id === id)?.amount ?? 0), 0)
    const pNet = [...selPlat].reduce((s, id) => {
      const p = S.platformTxns.find((t) => t.id === id)
      return s + (p ? (p.type === 'deposit' ? p.amount : -p.amount) : 0)
    }, 0)
    const nb = selBank.size, np = selPlat.size
    $('#sums').textContent = !nb && np
    ...
    $('#bar-hint').textContent =
      !nb && !np ? 'Select rows: bank + platform → Match. Bank only → tag or mark. Platform only → match cancelling deposit↔withdrawal, or paid off-platform.'
      : nb && np ? 'Match links these as one payment.'
      : nb >= 2 ? 'Match pairs bank rows that cancel out (in → forwarded out), or tag/mark them below.'
      : nb ? 'Pick the matching platform row on the right (best guesses highlighted), or tag/mark this row. A ~5% overage is usually the donation fee — use "Match, Δ → revenue".'
      : np >= 2 ? 'Match pairs a deposit ↔ withdrawal that cancel (same user, no bank movement) — or mark them paid off-platform.'
      : 'This platform txn was settled outside the imported bank accounts? Mark it paid off-platform.'
  }
  updateSums()

  $('#f-source').onchange = (e) => { filters.bankSource = e.target.value; render() }
  $('#f-cat').onchange = (e) => { filters.bankCat = e.target.value; render() }
  $('#f-btext').onchange = (e) => { filters.bankText = e.target.value; render() }
  $('#f-ptext').onchange = (e) => { filters.platText = e.target.value; render() }
  $('#f-small').onchange = (e) => { filters.hideSmall = e.target.checked; render() }
  ...
  main.querySelectorAll('input[data-plat]').forEach((cb) => {
    cb.onchange = () => { cb.checked ? selPlat.add(cb.dataset.plat) : selPlat.delete(cb.dataset.plat); updateSums() }
  })
  main.querySelectorAll('button[data-unmatch]').forEach((b) => {
    b.onclick = async () => { await api('/api/unmatch', { matchId: b.dataset.unmatch }); await loadState() }
  })
  ...
  const clearSel = () => { selBank.clear(); selPlat.clear(); suggested = [] }
  $('#clear-sel').onclick = () => { clearSel(); render() }
  ...
  const doMatch = async (kind, category, platformIds, remainderToRevenue) => {
    try {
      ...
      if ((e.message.includes('mismatch') || e.message.includes('do not cancel out')) && confirm(e.message + '\n\nForce match anyway?')) {
        await api('/api/match', { bankTxnIds: [...selBank], platformTxnIds: platformIds, kind, category, note: $('#m-note').value || undefined, force: true })
        clearSel()
        await loadState()
  ...
  $('#do-match').onclick = () => {
    const nb = selBank.size, np = selPlat.size
    if (!((nb && np) || nb >= 2 || np >= 2))
      return status('select bank+platform rows, ≥2 bank rows, or ≥2 platform rows that cancel', true)
  ...
  $('#do-op').onclick = async () => {
    ...
    try {
      await api('/api/match', { bankTxnIds: [], platformTxnIds: [...selPlat], kind: 'offplatform_payment', category: $('#op-cat').value, note: $('#m-note').value || undefined })
      clearSel()
      await loadState()
  ...
  $('#set-cat').onchange = async (e) => {
    ...
    await api('/api/category', { bankTxnIds: [...selBank], category: e.target.value === '__clear' ? '' : e.target.value })
    clearSel()
    await loadState()
  }
}
...
const ORG = { mercury_grants: 'manifund', stripe_manifund: 'manifund', coinbase: 'manifund', other: 'manifund', mercury_mox: 'mox', stripe_mox: 'mox' }
const CAT_LABELS = {
  payroll: 'Payroll',
  rent: 'Rent',
  cleaning: 'Cleaning',
...
function renderOverview() {
  const [ws, we] = windowRange()
  if (!ws || !we) { main.innerHTML = '<div class="section">Set a date window above.</div>'; return }
  ...
  for (const t of S.bankTxns) {
    ...
    if (cat === 'internal_transfer') {
      const partnerId = m && m.kind === 'internal_transfer' && m.bankTxnIds.find((id) => id !== t.id)
      const partner = partnerId && bankById.get(partnerId)
      if (partner && ORG[partner.source]) { residual += t.amount; continue } // counted<->counted nets out
    ...
    if (cat === 'other') {
      ...
      if (cat === 'revenue') add('in', 'Revenue', org, t.amount)
  ...
  for (const m of S.matches) {
    ...
    add('in', 'Revenue', org, m.revenueRemainder)
  ...
  const unmatchedDeps = S.platformTxns.filter((p) => p.type === 'deposit' && inWin(p.created_at.slice(0, 10)) && !mPlat.has(p.id))
  const unmatchedDepsSum = unmatchedDeps.reduce((s, p) => s + p.amount, 0)
  const unBank = S.bankTxns.filter((t) => inWin(t.date) && !t.matchId && !mb.has(t.id) && !t.category)
  ...
  const cell = (v) => `<td class="num ${v < 0 ? 'neg' : v > 0 ? 'pos' : ''}">${v ? fmt0(v) : ''}</td>`
  const groups = [
    ['Money in', 'in'],
    ['Money out', 'out'],
    ['', 'diff'],
  ...
  main.innerHTML = `
    ...
    <div class="section"><h2>Expected NAV change · ${ws} → ${we}</h2>
      <div class="tbl-wrap"><table>
        ...
        ${groups.map(([label, g]) => {
          const rs = entries.filter(([, r]) => r.group === g).sort((a, b) => Math.abs(total(b[1])) - Math.abs(total(a[1])))
          return rs.length ? (label ? `<tr><th>${label}</th><th></th><th></th><th></th></tr>` : '') +
            rs.map(([, r]) => `<tr><td>${r.label}</td>${cell(r.manifund)}${cell(r.mox)}${cell(total(r))}</tr>`).join('') : ''
        }).join('')}
