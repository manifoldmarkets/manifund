...
export function GeneratedAvatar(props: { uuid: string; size?: number }) {
  ...
  const [fromString, toString, iconString] = [
    uuid.substring(9, 13),
    uuid.substring(14, 18),
    uuid.substring(19, 23),
  ]
  const [fromNum, toNum, iconNum] = [
    parseInt(fromString, 16),
    parseInt(toString, 16),
    parseInt(iconString, 16),
  ]
  const [fromIdx, toIdx, iconIdx] = [
    fromNum % 36,
    toNum % 32,
    iconNum % emojis.length,
  ]
  const [fromColor, toColor, icon] = [
    fromColors[fromIdx],
    toColors[toIdx],
    emojis[iconIdx],
  ]
